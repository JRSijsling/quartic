...
function FlexesThroughPoint(f0,p);
    ...
    P2 := ProjectiveSpace(F,2); x1 := P2.1; x2 := P2.2; x3 := P2.3;
    R2 := CoordinateRing(P2); x1 := R2.1; x2 := R2.2; x3 := R2.3;
    ...
    if not (IsZero(F!2) or IsZero(F!3)) then
        ...
        R := PolynomialRing(F,4); l := R.1; m := R.2; s := R.3; t := R.4;
        A4lmst := Spec(R);
        A2rel := AffineSpace(R,2); y1 := A2rel.1; y2 := A2rel.2;
        P2rel := BaseExtend(P2,R); x1n := P2rel.1; x2n := P2rel.2; x3n := P2rel.3;
    ...
    P2FF:=BaseExtend(P2,FF); x1FF := P2FF.1; x2FF := P2FF.2; x3FF := P2FF.3;
...
function TrivializeAlgebra(B,pt)
    ...
    //P2 := ProjectiveSpace(A,2); x := P2.1; y := P2.2; z := P2.3;
...
function TransvectantGeneral(polys, l : normalize := false)
    n := #polys;
    R := Universe(polys);
